...
topCard.classList.add('flipping-forward');

setTimeout(() => {
topCard.classList.remove('flipping-forward');
deck.appendChild(topCard); // 把第一張放到最後面
...
lastCard.classList.add('flipping-backward');
deck.insertBefore(lastCard, cards[0]); // 把最後一張抽到最前面
updateCardZIndex(deck);
...
card.style.transform = `translate(${index * 6}px, ${index * 6}px) scale(1)`;
card.style.opacity = index < 3 ? '1' : '0'; // 只顯示前三張堆疊感，其餘隱藏防混亂
...
// Intersection Observer 滾動側邊導覽監控
const observerOptions = { root: null, rootMargin: '0px', threshold: 0.6 };
const observer = new IntersectionObserver((entries) => {
entries.forEach(entry => {
if (entry.isIntersecting) {
const id = entry.target.getAttribute('id');
document.querySelectorAll('.habitant-quick-nav a').forEach(navLink => navLink.classList.remove('active'));
const activeLink = document.querySelector(`.habitant-quick-nav a[href="#${id}"]`);
if (activeLink) { activeLink.classList.add('active'); }
}
});
}, observerOptions);

document.querySelectorAll('.deck-container').forEach(section => observer.observe(section));
</script>
...
/* 🎬 往前翻動畫：從左上角優雅飄回原位 */
.content-card.flipping-backward { animation: backIn 0.4s cubic-bezier(0.25, 1, 0.5, 1) forwards; }
@keyframes backIn {
0% { transform: translate(-60px, -40px) rotate(-6deg) scale(0.92); opacity: 0; }
100% { transform: translate(0, 0) scale(1); opacity: 1; }
}

.split-layout { flex-direction: row; gap: 50px; text-align: left; }
.card-image { flex: 1.2; display: flex; align-items: center; height: 100%; }
.card-image img { width: 100%; height: 100%; max-height: 420px; object-fit: cover; border-radius: 2px; }
.card-text { flex: 1; text-align: left; display: flex; flex-direction: column; justify-content: center; }
.habitant-tag { font-family: 'Lora', serif; font-size: 0.8rem; letter-spacing: 2px; color: #A67C52; margin-bottom: 15px; display: block; text-align: left; text-transform: uppercase; }
.content-card h3 { font-family: 'Lora', serif; font-size: 2.3rem; margin-bottom: 20px; color: #1a1a1a; text-align: left; line-height: 1.2; }
.content-card p { font-family: 'Lora', serif; font-size: 1.05rem; line-height: 1.8; color: #333; text-align: left; margin: 0; }
.flip-hint { margin-top: auto; font-size: 0.75rem; color: #A67C52; font-style: italic; opacity: 0.6; text-align: left; padding-top: 20px; letter-spacing: 1px; }

/* 右側快速導覽選單 */
.habitant-quick-nav { position: fixed; top: 50%; right: 40px; transform: translateY(-50%); z-index: 1000; display: flex; flex-direction: column; gap: 20px; text-align: right; background: rgba(244, 241, 234, 0.4); padding: 20px 10px; border-radius: 4px; backdrop-filter: blur(5px); }
.habitant-quick-nav a { font-family: 'Lora', serif; color: #5d4037; text-decoration: none; font-size: 0.75rem; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; transition: all 0.3s ease; display: block;}
.habitant-quick-nav a:hover { color: #A67C52; transform: scale(1.1); }
.habitant-quick-nav a.active { color: #A67C52; font-weight: 900; transform: scale(1.1) translateX(-5px); }
.habitant-quick-nav a.active::after { content: " •"; font-size: 1.2rem; vertical-align: middle; }

@media (max-width: 1024px) {
.cards-deck { width: 88%; max-width: 380px; height: 72vh; min-height: 520px; max-height: 640px; margin: 40px auto; }
.nav-zone-left { width: 50%; height: 100%; z-index: 999; }
.nav-zone-right { width: 50%; height: 100%; z-index: 999; }
.content-card { padding: 20px; display: flex; flex-direction: column; justify-content: flex-start; overflow-y: auto; -webkit-overflow-scrolling: touch; }
.split-layout { display: flex; flex-direction: column; gap: 16px; align-items: stretch; justify-content: flex-start; height: 100%; }
.card-image { width: 100%; flex: 0 0 auto; height: 42%; overflow: hidden; border-radius: 2px; }
.card-image img { width: 100%; height: 100%; object-fit: cover; }
.card-text { width: 100%; flex: 1; display: flex; flex-direction: column; text-align: left; padding: 0 4px; }
.card-text .habitant-tag, .card-text .season-tag { font-size: 0.75rem; margin-bottom: 6px; }
.content-card h3 { font-size: 1.25rem; margin: 0 0 10px 0; line-height: 1.35; word-break: break-word; }
.content-card p { font-size: 0.92rem; line-height: 1.6; margin: 0 0 12px 0; color: #444; }
.card-text .flip-hint { margin-top: auto; padding-top: 8px; font-size: 0.7rem; opacity: 0.6; text-align: center; }
.habitant-quick-nav { top: 70px; right: 50%; transform: translateX(50%); flex-direction: row; width: 90%; max-width: max-content; overflow-x: auto; white-space: nowrap; padding: 6px 12px; background: rgba(244, 241, 234, 0.92); }
}
</style>
